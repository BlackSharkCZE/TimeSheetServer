<template>
  ...
  <div class="field">
    <label for="company">{{props.label}} <span v-if="vualidate.required">*</span></label>
    <Dropdown id="company"
              :class="{'p-invalid':vualidate.$invalid && submitted}"
              v-model="vualidate.$model" :options="companies" optionLabel="name" :filter="true"
    ...
    <small v-if="vualidate.$invalid && submitted"
           class="p-error">{{ vualidate.required.$message.replace('Value', 'Company') }}</small>
  </div>
