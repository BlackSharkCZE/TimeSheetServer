<template>
  ...
  <DataTable
    ...
    <Column field="name" header="PROJECT NAME" filterMatchMode="startsWith" ref="companyName">
...
function loadData(filterData: any = {}) {
  loading.value = true
  axios?.post("/datatable/projects?sort=name&page=" + pageIndex.value + "&pageSize=" + pageSize.value, filterData)
      .then((response) => {
    ...
    console.error('Can not load projects', error)
    loading.value = false
  })
}
